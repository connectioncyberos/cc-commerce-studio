...
type VideoScriptCardProps = {
  videoScript: VideoScript;
};

export function VideoScriptCard({ videoScript }: VideoScriptCardProps) {
  ...
  async function handleDelete() {
    const confirmed = window.confirm(
      `Excluir o roteiro "${videoScript.title}"? Essa ação não pode ser desfeita.`,
    );
    if (!confirmed) return;
      ...
      <Card>
        <form onSubmit={handleSave} className="space-y-3">
          <div>
            ...
            <Input name="title" defaultValue={videoScript.title} required />
          </div>
          ...
          <div>
            ...
            <Textarea name="script" defaultValue={videoScript.script ?? ""} rows={8} />
          </div>
    ...
    <Card>
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-sm capitalize text-neutral-500">{videoScript.status}</p>
        </div>
